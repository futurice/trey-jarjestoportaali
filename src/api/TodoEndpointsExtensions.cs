...
public static class TodoEndpointsExtensions
{
    ...
    public static async Task<Ok<IEnumerable<TodoList>>> GetLists(ListsRepository repository, int? skip = null,
        int? batchSize = null)
    {
    ...
    public static async Task<IResult> UpdateList(ListsRepository repository, string listId, CreateUpdateTodoList list)
    {
        ...
        if (existingList == null) return TypedResults.NotFound();
    ...
    {
        if (await repository.GetListAsync(listId) == null) return TypedResults.NotFound();
    ...
    public static async Task<IResult> GetListItems(ListsRepository repository, string listId, int? skip = null,
        int? batchSize = null)
    {
        if (await repository.GetListAsync(listId) == null) return TypedResults.NotFound();
    ...
    public static async Task<IResult> CreateListItem(ListsRepository repository, string listId,
        CreateUpdateTodoItem item)
    {
        if (await repository.GetListAsync(listId) == null) return TypedResults.NotFound();
    ...
    public static async Task<IResult> UpdateListItem(ListsRepository repository, string listId, string itemId,
        CreateUpdateTodoItem item)
    {
    ...
    {
        if (await repository.GetListItemAsync(listId, itemId) == null) return TypedResults.NotFound();
    ...
    public static async Task<IResult> GetListItemsByState(ListsRepository repository, string listId, string state,
        int? skip = null, int? batchSize = null)
    {
        if (await repository.GetListAsync(listId) == null) return TypedResults.NotFound();
